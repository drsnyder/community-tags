...
The `home-routes` handler in the `community-tags.routes.home` namespace
...
The `home-page` function will in turn call the `community-tags.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `community-tags.handler` namespace:
...
Request middleware functions are located under the `community-tags.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `community-tags.dev-middleware` namespace found in
